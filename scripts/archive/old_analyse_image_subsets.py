...
def getImageSequence(threshold, num_feats, all_feat_flat, ranking_type, initial_image_id_set):
...
def getShapeCoverage(ranking_type, image_2_shapes_map, num_orderings_needed, initial_image_id_set = None):
    ...
    for i in tqdm(range(num_orderings_needed)):
        ordering, temp_num_active_neurons = getImageSequence(0, tot_num_shapes, neighs_tensor, ranking_type, initial_image_id_set)
...
def main(args):
    ...
    for pars in allowed_parallels: # TODO allowed_oaralels
        ...
        for s in ["worst", "global", "best"]:
            ...
            ordering_dir = f'{args.data_json}/ordering-start-worst/{s}/{dec_short}-{pars}/{q[8:]}'
            ordering_path = f'{ordering_dir}/{args.images_min}-{args.images_max}.pt'
